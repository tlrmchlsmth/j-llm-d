...
# QPS Sweep with vLLM Log Collection (auto-detects agg vs disagg)
# Usage: ./sweep_qps_with_kv_transfer_logs.sh <request_rate> <output_dir>
# Example: ./sweep_qps_with_kv_transfer_logs.sh 8 results/1p1d_8
#
# Writes to <output_dir>/:
#   result_qps<N>.txt   - benchmark output (also printed to terminal)
#   decode_qps<N>.log   - vLLM logs from the decode (or agg) pod
#   prefill_qps<N>.log  - vLLM logs from the prefill pod (disagg only)
#   metrics_qps<N>.txt  - Prometheus metric deltas (prefill pods for disagg, agg pod for agg)
#
# Auto-discovers model server pods in the namespace, detects whether the
# deployment is aggregated (glm-agg) or disaggregated (glm-disagg), captures
# vLLM logs for the duration of the sweep, then stops.

set -euo pipefail

if [ $# -lt 2 ]; then
    echo "Usage: $0 <request_rate> <output_dir>"
    echo "Example: $0 8 results/1p1d_8"
    exit 1
fi
...
DECODE_LOG="$OUT_DIR/decode_qps${QPS}.log"
PREFILL_LOG="$OUT_DIR/prefill_qps${QPS}.log"
RESULT_LOG="$OUT_DIR/result_qps${QPS}.txt"
METRICS_LOG="$OUT_DIR/metrics_qps${QPS}.txt"
...
# Snapshot Prometheus metrics before benchmark
...
echo ""
echo "=============================================="
echo "Running QPS Sweep via Poker Pod (timeout: ${BENCHMARK_TIMEOUT_SEC}s)"
echo "  Target: $TARGET | QPS: $QPS"
echo "=============================================="
echo ""
echo "Running: just TARGET_DURATION=30 MIN_PROMPTS=120 sweep-qps $TARGET 4096 256 $QPS"
echo ""

timeout "$BENCHMARK_TIMEOUT_SEC" kubectl exec -n "$NAMESPACE" poker -- /bin/zsh -c "cd /app && just TARGET_DURATION=30 MIN_PROMPTS=120 sweep-qps $TARGET 4096 256 $QPS" 2>&1
BENCH_EXIT=$?

echo ""
if [ "$BENCH_EXIT" -eq 124 ]; then
    echo "=============================================="
    echo "Sweep did not finish within ${BENCHMARK_TIMEOUT_SEC}s; killed."
    echo "=============================================="
else
    echo "=============================================="
...
if [ -n "$METRICS_IPS" ] || [ -n "$DECODE_IPS" ]; then
    echo ""
    echo "=============================================="
    echo "Prometheus Metrics Summary (qps=$QPS)"
    echo "=============================================="
    python3 -c "
import os
