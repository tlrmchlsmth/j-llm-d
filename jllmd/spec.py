...
class RoleSpec(BaseModel):
    ...
    def validate_parallelism(self) -> "RoleSpec":
        if self.gpus_per_pod % self.tensor_parallel_size != 0:
            raise ValueError("gpus_per_pod must be divisible by tensor_parallel_size")
        if self.data_parallel.enabled:
            assert self.data_parallel.local_size is not None
            if self.data_parallel.local_size * self.tensor_parallel_size != self.gpus_per_pod:
                raise ValueError("data_parallel.local_size * tensor_parallel_size must equal gpus_per_pod")
        return self
